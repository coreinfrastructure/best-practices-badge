...
file 'license_finder_report.html' => ['Gemfile.lock', 'doc/dependency_decisions.yml'] do
...
task :restore_en do
  FileUtils.mv Rails.root.join('config', 'locales', 'en.yml.ORIG'), Rails.root.join('config', 'locales', 'en.yml')
end
...
def real_search_email(email)
  ...
  results = User.where(email: email).select('id, name, encrypted_email, encrypted_email_iv').pluck(:id, :name)
  puts results
end
