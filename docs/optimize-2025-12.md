...
almost all pages are in this hierarchy. Thus, for us, it's important to
optimize these pages in particular.

We also want to add support for baseline criteria. However, adding that
without any other changes would
increase the burden (we'd have more pages to serve), so we really
...
* Optimize the Rails router. When this application receives
...
* When loading project data, we *only* load the fields we intend to use.
...
* Change the edit HTML pages to *only* show specific sections.
...
* Convert all `X_status` internal values into integers 0..3 instead of strings.
  ...
  Conceptually this was easy, but it practice it had a lot of fiddly
  changes because criteria status is *all* over the application.
...
* Convert all `X_justification` values
  ...
  See the [empty justification string as null](empty-justification-string-as-null.md) document for more information.
* In many places, pre-compute a constant *once*, then use it, instead
...
* Optimize common cases. E.g. we log when we do our first gc compacting,
...
* Don't call the markdown processor for trivial cases where it makes
  no difference. In many cases only trivial strings are provided, and this
  avoids unnecessary work to process them.
* We changed the run-time configuration to do a garbage collection
  compaction every hour, instead of every two. This meants that there
