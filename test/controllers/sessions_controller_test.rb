...
class SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should get new' do
    ...
    assert_match(/<input [^>]+ type="email" name="session\[email\]" id="session_email" /, @response.body)
    # Do a pickier check of the results using XPath selectors
    assert_select('form/input[type="email"][name="session[email]"][id="session_email"]')
    ...
    assert_select('form/input[type="password"][name="session[password]"][id="session_password"]')
  ...
  test 'Simple login (directly)' do
    ...
    post '/en/login', params: { session: { provider: 'local', email: 'test@example.org', password: 'password' } }
    assert_response :redirect
    assert_redirected_to root_url
    follow_redirect!
