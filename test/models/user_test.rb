...
class UserTest < ActiveSupport::TestCase
  ...
  # is never reached during normal test runs; exercise it explicitly here.

  test 'hex_key_for falls back to test key when EMAIL_ENCRYPTION_KEY absent' do
    saved = ENV.delete('EMAIL_ENCRYPTION_KEY')
    assert_equal User::TEST_EMAIL_ENCRYPTION_KEY,
                 User.send(:hex_key_for, env_var: 'EMAIL_ENCRYPTION_KEY',
                                          test_value: User::TEST_EMAIL_ENCRYPTION_KEY, env_test: false)
  ensure
    ENV['EMAIL_ENCRYPTION_KEY'] = saved if saved
  end

  test 'hex_key_for uses EMAIL_ENCRYPTION_KEY env var when set' do
  ...
  test 'hex_key_for falls back to test key when EMAIL_BLIND_INDEX_KEY absent' do
    saved = ENV.delete('EMAIL_BLIND_INDEX_KEY')
    assert_equal User::TEST_EMAIL_BLIND_INDEX_KEY,
                 User.send(:hex_key_for, env_var: 'EMAIL_BLIND_INDEX_KEY',
                                          test_value: User::TEST_EMAIL_BLIND_INDEX_KEY, env_test: false)
  ensure
    ENV['EMAIL_BLIND_INDEX_KEY'] = saved if saved
  end

  test 'hex_key_for uses EMAIL_BLIND_INDEX_KEY env var when set' do
