...
GitHub Actions run a few additional checks that aren't part of the
default rake task and thus don't run locally: Brakeman
(`.github/workflows/brakeman.yml`), CodeQL, and the OSS license check
...
`license_finder` isn't in the `Gemfile`; it's a CI-only analyzer (like
Brakeman), run by `.github/workflows/license_finder.yml` on every pull
request and push to main. To run it locally, install it directly
(`gem install license_finder`) rather than through `bundle exec`.
...
you can also run 'license_finder report --format html > license_finder_report.html'
and read the report it generates, or download the report artifact from a
recent run of `.github/workflows/license_finder.yml`.
