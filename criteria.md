...
- At least one of the primary developers MUST know how to design secure software.  These include understanding the following design principles from Saltzer and Schroeder: economy of mechanism (have a simple and small design), fail-safe defaults (deny by default), complete mediation (be non-bypassable), open design (the software should be secure even if attackers know its design), least privilege (processes should operate with minimum privilege), least common mechanism (the design should minimize the mechanisms common to multiple users), and psychological acceptability (be easy to use).  The developer must also know the value of limiting the attack surface, why and how to do input validation, and the advantages of whitelists over blacklists in input data validation.  <!-- The S and S principle "separation of privilege" (where feasible, a protection mechanism that requires more than one key to unlock is more robust) is great, but rarely done, so it is not required for a "basic" badge --> [know-secure-design]
