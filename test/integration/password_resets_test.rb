...
class PasswordResetsTest < ActionDispatch::IntegrationTest
  ...
  test 'password resets' do
    ...
    # Invalid email
    post password_resets_path, params: { password_reset: { email: '' }, locale: :en }
    assert_not flash.empty?
    assert_redirected_to root_url(locale: :en)
    # Valid email, github user
    post password_resets_path, params: { password_reset: { email: @ghuser.email }, locale: :en }
    assert_equal 0, ActionMailer::Base.deliveries.size
    assert_not flash.empty?
    assert_redirected_to root_url(locale: :en)
    # Valid email
    post password_resets_path, params: { password_reset: { email: @user.email }, locale: :en }
    ...
    get edit_password_reset_path(user.reset_token, email: user.email, locale: :en)
    assert_redirected_to root_url(locale: :en)
    user.toggle!(:activated)
  ...
  test 'expired token' do
    get new_password_reset_path(locale: :en)
    post password_resets_path(locale: :en), params: { password_reset: { email: @user.email }, locale: 'en' }
