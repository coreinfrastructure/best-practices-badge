...
This document describes how the badge application handles automated field
proposals — both from external tools (URL query string) and from the
internal Chief/detective system. It covers where proposals originate, how
...
### 1. URL Query-String Proposals (external)
...
### 2. Chief / Detective Proposals (internal)
...
- The **first edit** of each badge section (before the section has been
  saved), or when `?reanalyze` is in the URL.
...
```ruby
{
  field_sym => {
    ...
    proposed_justification:  # String or nil — _status fields only; the
                             # proposed justification text shown in the ≠ popover
  }
}
```
...
|---|---|---|---|---|
| unparsable or `?` proposed | — | — | Skip (pre-screened in `build_url_proposals`) | none |
| blank / `?` | any real value | either | Apply | yellow |
| real, equals proposed | — | — | Skip (no-op) | none |
| real, differs | differs | no | Keep user value | ≠ |
| real, differs | differs | yes | Overwrite | orange |
...
| present, differs | differs | no | other (name, license…) | Keep user value | ≠ |
| present, differs | differs | yes | any | Overwrite | orange |
...
Runs on **every** edit-page visit (after `run_first_edit_automation_if_needed`).
Results merge into any existing highlight state via `||=`, so URL params
can extend or override Chief results.
