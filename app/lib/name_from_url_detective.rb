...
class NameFromUrlDetective < Detective
  ...
  def analyze(_evidence, current)
    ...
    name_in_homepage_url_domain = %r{\Ahttps?://(www\.)?([A-Za-z0-9-]+)\.([A-Za-z0-9._-]*)/?\Z}
    ...
    if homepage_url.present?
      ...
      if finding && finding[2].present?
        ...
        if finding
          @results[:name] = { value: finding[1], confidence: 1, explanation: "The project URL's tail suggests this." }
        end
      end
    end
    if !@results.key?(:name) && repo_url.present?
      ...
      if finding
        @results[:name] = { value: finding[1], confidence: 1, explanation: "The repo URL's tail suggests this." }
      end
    end
    @results
  end
