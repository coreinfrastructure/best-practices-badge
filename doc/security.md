...
* After synchronization, and on every test run (including deployment to a tier),
  *every* text segment (including English) is checked, including to
  ensure that *only* an acceptlisted set of HTML tags
...
* Limited attack surface (the attack surface, the set of the different
  ...
  trivial to validate).
* Input validation with acceptlists
  (inputs should typically be checked to determine if they are valid
  before they are accepted; this validation should use acceptlists
  ...
  input validation is done with acceptlists through controllers and models.
  Parameters are first checked in the controllers using the Ruby on Rails
  "strong parameter" mechanism, which ensures that only an acceptlisted set
  ...
  an acceptlist of values allowed for that criterion.
  ...
  URLs are also limited by length and an acceptlisted regex, which counters
...
4. *Redirection and Files.*
   ...
   the id first (which is an acceptlist).
...
7. *Injection.*
   We use acceptlists to validate project data entered into the system.
...
2. Inventory of Authorized and Unauthorized Software.
   ...
   package managers.
   We do not use an acceptlisting mechanism because it would be pointless;
